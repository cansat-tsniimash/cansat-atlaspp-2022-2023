...
int bb_read(uint32_t addr, uint8_t* buf, uint8_t size)
{
	...
	HAL_Delay(10);
	cmd = I2C_LINK_CMD_GET_SIZE;
	rc = HAL_I2C_Master_Transmit(&hi2c1, 0x77<<1, &cmd, 1, 1);
	if (rc == HAL_BUSY)
	{
		I2C_ClearBusyFlagErratum(&hi2c1, 100);
	}
	...
	rc = HAL_I2C_Master_Receive(&hi2c1, 0x77<<1, (uint8_t *)&size_mes, 2, 1);
	if (rc == HAL_BUSY)
	{
		I2C_ClearBusyFlagErratum(&hi2c1, 100);
	}
	...
	if(size_mes >= size)
		rc = HAL_I2C_Master_Receive(&hi2c1, 0x77<<1, buf, size, 1);
	else
		rc = HAL_I2C_Master_Receive(&hi2c1, 0x77<<1, buf, size_mes, 1);
	if (rc == HAL_BUSY)
	{
		I2C_ClearBusyFlagErratum(&hi2c1, 100);
	}
